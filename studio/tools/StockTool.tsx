    ...
    async (id: string, key: string, raw: string) => {
      ...
      await (
        value === ''
          ...
          : patch.set({
              [`variants[_key=="${key}"].stock`]: Math.max(0, Math.floor(Number(value))),
            })
      ).commit()
      setBusy(null)
      load()
  ...
  return (
    ...
    >
      ...
      <p style={{margin: '8px 0 24px', color: '#666', fontSize: 14, lineHeight: 1.5}}>
        How many of each size you have. A size comes off the shop by itself at zero, and returns
        when you put a number in. Leave a box empty to stop counting that size.
      </p>
      ...
      {rows.map((row) => (
        ...
        >
          ...
          {(row.variants ?? []).map((variant) => {
            const label = [variant.size, variant.heat].filter(Boolean).join(', ') || 'One size'
            const out = variant.stock === 0
            const low = typeof variant.stock === 'number' && variant.stock > 0 && variant.stock <= 5
            ...
            return (
              ...
              >
                ...
                {out ? (
                  <span style={{color: '#c0392b', fontSize: 12, fontWeight: 600}}>Sold out</span>
                ) : null}
                {low ? (
                  <span style={{color: '#b7791f', fontSize: 12, fontWeight: 600}}>Low</span>
                ) : null}
                ...
                <input
                  ...
                  defaultValue={typeof variant.stock === 'number' ? String(variant.stock) : ''}
                  disabled={busy === `${row._id}:${variant._key}`}
                  onBlur={(event) => save(row._id, variant._key, event.currentTarget.value)}
                  style={{
                    width: 110,
                    padding: '6px 8px',
